...
import com.baolinetworktechnology.shejiquan.domain.SJQeaseuser;
...
public class DemoHelper {
	...
	public void init(Context context) {
        ...
        getEaseUserUI(context);
    }
    private  List<SJQeaseuser> UserUIList;
    protected void getEaseUserUI(Context context){
        if(UserUIList==null){
            UserUIList =new ArrayList<>();
        }
        DBHelper helper=new DBHelper(context,DBHelper.ea_NAME);
        Cursor cursor = helper.esaequery();
        while (cursor.moveToNext()) {
            SJQeaseuser sortmodel = new SJQeaseuser();
            sortmodel.setNick(cursor.getString(cursor.getColumnIndex("Nick")));
            sortmodel.setAvatar(cursor.getString(cursor.getColumnIndex("Avatar")));
            sortmodel.setLoginName(cursor.getString(cursor.getColumnIndex("LoginName")));
            UserUIList.add(sortmodel);
        }
        cursor.close();
        helper.close();
    }
    ...
    protected void setEaseUIProviders() {
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                Intent intent = new Intent(appContext, XiangQinActivity.class);
	...
	private EaseUser getUserInfo(String username){
	    ...
	    if(getRobotList() != null) {
	    	 easeUser = getRobotList().get(username);
	     }
	    ...
	    if(easeUser == null){
	      easeUser = new EaseUser(username);
            ...
            if(UserUIList!=null){
                for (int c = 0; c < UserUIList.size(); c++){
                    String userguid=UserUIList.get(c).getLoginName();
                    if(userguid.equals(username)){
                        easeUser.setNick(UserUIList.get(c).getNick());
                        easeUser.setAvatar(UserUIList.get(c).getAvatar());
                    }
                }
                return easeUser;
            }
    	...
    	messageListener = new EMMessageListener() {
			...
			public void onMessageReceived(List<EMMessage> messages) {
				 for (EMMessage message : messages) {
				        ...
				        //************接收并处理扩展消息***********************
				        String userName = message.getStringAttribute("kIMExtDictionaryUserNickName", "");
				        String userPic = message.getStringAttribute("kIMExtDictionaryUserLogo", "");
                        String UserGuid = message.getStringAttribute("kIMExtDictionaryUserGUID", "");
                        UserGuid = CommonUtils.removeAllSpace(UserGuid);
				        String hxIdFrom = message.getFrom();
                        EMConversation  conversation = EMClient.getInstance().chatManager().getConversation(
                                hxIdFrom, EaseCommonUtils.getConversationType(1), true);
                         conversation.setExtField(UserGuid);
                        ...
                        //联系人信息存入本地数据库
                         ...
                         values.put("Nick",userName);
                         values.put("Avatar", userPic);
                         values.put("LoginName", UserGuid);
                         DBHelper helper=new DBHelper(appContext,DBHelper.ea_NAME);
                         helper.easedelete(UserGuid);
                         helper.esaeinsert(values);
                         helper.close();
//                         SJQeaseuser sjQeaseuser=new SJQeaseuser();
//                         sjQeaseuser.setNick(userName);
//                         sjQeaseuser.setAvatar(userPic);
//                         CacheUtils.cacheStringData(appContext,UserGuid ,
//                                 sjQeaseuser.toString());
				        if (isGroupsSyncedWithServer()) {
...
//                        intent.setAction("ReadMag");
  ...
  public void addSyncGroupListener(DataSyncListener listener) {
   ...
   public void asyncFetchBlackListFromServer(final EMValueCallBack<List<String>> callback){
    ...
    public void putlianxi(String LoginName,String NickName,String UserLogo) {
    	    EaseUser easeUser = new EaseUser(LoginName);
	        easeUser.setAvatar(UserLogo);
	        easeUser.setNick(NickName);
	        // 存入内存
	        getContactList();
	        contactList.put(LoginName, easeUser);
	        // 存入db
	        UserDao dao = new UserDao(appContext);
	        List<EaseUser> users = new ArrayList<EaseUser>();
	        users.add(easeUser);
            ...
            values.put("Avatar", UserLogo);
            values.put("LoginName", LoginName);
            DBHelper helper=new DBHelper(appContext,DBHelper.ea_NAME);
            helper.easedelete(LoginName);
            helper.esaeinsert(values);
            helper.close();
//            SJQeaseuser sjQeaseuser=new SJQeaseuser();
//            sjQeaseuser.setNick(NickName);
//            sjQeaseuser.setAvatar(UserLogo);
//            CacheUtils.cacheStringData(appContext,LoginName ,
//                sjQeaseuser.toString());
	        // 通知listeners联系人同步完毕
	        notifyContactsSyncListener(true);
	        if (isGroupsSyncedWithServer()) {
