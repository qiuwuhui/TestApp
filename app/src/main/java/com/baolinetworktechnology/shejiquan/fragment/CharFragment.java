 ...
 */
public class CharFragment extends BaseMainFragment{
    public  CharFragment() {
    }
    ...
    private void showDesignData(final DesignerDetail data) {
    ...
    public void GetDesigner() {
        String url = ApiUrl.GET_DESIGNER_INFO + designerGudi;
        RequestCallBack<String> callBack = new RequestCallBack<String>() {
            @Override
            public void onSuccess(ResponseInfo<String> responseInfo) {
                DesignerDetailBean detailBean = CommonUtils.getDomain(
                        responseInfo, DesignerDetailBean.class);

                if(detailBean.success){
                    if(detailBean!=null){
                      showDesignData(detailBean.data);
                    }
                }
            }
            @Override
        ...
        getHttpUtils().send(HttpRequest.HttpMethod.GET, ApiUrl.API + url, getParams(url), callBack);
    }
    ...
    protected void lazyLoad() {
        GetDesigner();
        if (SJQApp.user != null){
            new NetMessage().GetSAVECONSULT(new OnCallBackBean<ReadMessageBean>() {
                @Override
                public void onNetStart() {
                }
                @Override
                public void onSuccess(ReadMessageBean bean) {
                }
                @Override
