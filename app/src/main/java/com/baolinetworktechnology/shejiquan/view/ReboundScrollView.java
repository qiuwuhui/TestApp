...
public class ReboundScrollView extends ScrollView {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		mOriginalRect.set(mContentView.getLeft(), mContentView.getTop(),
...
//		}
		...
		case MotionEvent.ACTION_UP:
			...
			TranslateAnimation anim = new TranslateAnimation(0, 0,
					mContentView.getTop(), mOriginalRect.top);
		...
		case MotionEvent.ACTION_MOVE:
			...
			// 是否应该移动布局
			boolean shouldMove = (mCanPullDown && deltaY > 0) // 可以下拉， 并且手指向下移动
					|| (mCanPullUp && deltaY < 0) // 可以上拉， 并且手指向上移动
					|| (mCanPullUp && mCanPullDown); // 既可以上拉也可以下拉（这种情况出现在ScrollView包裹的控件比ScrollView还小）
			...
			if (shouldMove) {
				...
				// 随着手指的移动而移动布局
				mContentView.layout(mOriginalRect.left,
						mOriginalRect.top + offset, mOriginalRect.right,
						mOriginalRect.bottom + offset);

				mIsMoved = true; // 记录移动了布局
			}
