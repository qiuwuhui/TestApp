...
import com.baolinetworktechnology.shejiquan.domain.GongLueBean;
...
public class ListViewNews extends MyListView implements OnClickListener {
	...
	public ListViewNews(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext=context;
		initView();
	}

	public ListViewNews(Context context) {
		super(context);
	...
	public void loadData(final boolean falg) {
		...
		String url="";
		if(mKeyWord==null){
	            url = AppUrl.DECORATIONGTLIST+ClassID+"&orders=12"+"&markName=0"+"&PageSize=15"+"&currentPage="+mPageIndex;
		 }else{
			try {
			    url ="/api/services/app/news/GetPageList?"+ "currentPage=" + mPageIndex+"&ClassID=0"+"&orders=12"
