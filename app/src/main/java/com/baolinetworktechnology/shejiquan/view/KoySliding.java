 ...
 * 
 */
public class KoySliding extends FrameLayout implements OnPageChangeListener {
	private ViewPager fs_vp;// 图片
	//private TextView tv_pager_description;// 描述信息
	private LinearLayout ll_pager_point_group;// 点
	private LunBoBean listData;
	private SlidAdapter slidAdapter;

	public KoySliding(Context context) {
		super(context);
		initView();
	}

	public KoySliding(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}
	...
	public void setDatas(LunBoBean listData) {
	...
	private class SlidAdapter extends PagerAdapter implements OnClickListener {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			if (iv.getTag() == null
				...
				bitmapUtils.display(iv, listData.result.get(position).path);
		...
		public void onClick(View v) {
			Items data = (Items) v.getTag();
			if(data.url.equals("activity")){
				Intent intent = new Intent(getContext(), WebActivity.class);
				String url="";
				if(SJQApp.user==null){
					url="http://m.sjq315.com/activity/android/entrustorder?IsAPP=1";					
				}else{
					url="http://m.sjq315.com/activity/android/entrustorder?IsAPP=1" +
							"&UserGUID="+SJQApp.user.guid;
				}
				intent.putExtra(WebActivity.URL,url);
				getContext().startActivity(intent);
			}else if(data.url.equals("designerlist")){
				Intent intent = new Intent(getContext(), MyFragmentActivity.class);
				intent.putExtra(MyFragmentActivity.TYPE, MyFragmentActivity.MAIN_DESIGN);
				getContext().startActivity(intent);
			}else if(data.url.equals("caselist")){
				Intent intent = new Intent(getContext(), MyFragmentActivity.class);
				intent.putExtra(MyFragmentActivity.TYPE, MyFragmentActivity.EntrustFastFragment);
				getContext().startActivity(intent);
			}else if(data.url.equals("forumlist")){
				Intent intent = new Intent();
				intent.setAction("qiehuan");
				getContext().sendBroadcast(intent);
			}else{
				Intent intent = new Intent(getContext(), WebActivity.class);
				intent.putExtra(WebActivity.URL, data.url);
				intent.putExtra(WebActivity.IMAGE_URL, data.path);
				getContext().startActivity(intent);
			}
