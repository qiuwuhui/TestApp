...
public class ScrollerNumberPicker extends View {
	...
	private void initData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			itmItemObject.itemText = title;
			itmItemObject.itemTextSub = title;
			if (showSub) {
				if (title.length() > 5)
					itmItemObject.itemTextSub = title.subSequence(0, 4) + "...";
			}
			itmItemObject.x = 0;
	...
	public void setDefault(int index, boolean b) {
		if (index >= itemList.size()) {
			index = 0;
		}
		if (itemList.size() == 0)
		...
		if (b) {
			if (onSelectListener != null)
				onSelectListener.endSelect(index, "" + index);
		}
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			textPaint.getTextBounds(itemTextSub, 0, itemTextSub.length(),
					textRect);
			// 判断是否可视
			if (!isInView())
				return;
			...
			// 绘制内容
			canvas.drawText(itemTextSub,
					x + controlWidth / 2 - textRect.width() / 2, y + move
							+ unitHeight / 2 + textRect.height() / 2, textPaint);
